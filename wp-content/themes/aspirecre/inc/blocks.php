...
add_action( 'after_setup_theme', static function (): void {
	add_editor_style( array( 'style.css', 'assets/editor.css', 'assets/corporate.css' ) );
} );
...
add_filter( 'render_block_core/image', static function ( string $content, array $block ): string {
	...
	if ( ! array_intersect( array( 'corp-houston-image', 'corp-management-image' ), $classes ) ) { return $content; }
	$html = new WP_HTML_Tag_Processor( $content );
	if ( $html->next_tag( 'IMG' ) ) {
		$html->set_attribute( 'loading', 'lazy' );
		$html->set_attribute( 'decoding', 'async' );
		$html->set_attribute( 'sizes', '(max-width: 700px) calc(100vw - 40px), (max-width: 1440px) 42vw, 604px' );
	}
