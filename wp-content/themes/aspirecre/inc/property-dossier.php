<?php
/** Single-property presentation only. Aspire Core owns all field definitions and records. */
defined('ABSPATH') || exit;
final class AspireCRE_Property_Dossier {
 ...
 public static function data(int $id): array {
  ...
  $feature=null;foreach(Aspire_Atlas::collection()['features'] as $item){if($item['id']===$id){$feature=$item;break;}}
  $city=$text('city');$state=$text('state');$zip=$text('postal_code');$title=sanitize_text_field(get_post_field('post_title',$id));
  // Atlas excludes unmapped/inactive properties. Match its suffix-only presentation rule for these single pages.
  if(!$feature&&$city!==''&&$state!==''){$suffix='/(?:,\s*|\s+)'.preg_quote($city,'/').'(?:\s*,\s*|\s+)'.preg_quote($state,'/').($zip!==''?'(?:\s+'.preg_quote($zip,'/').')?':'').'\s*$/iu';$title=trim((string)preg_replace($suffix,'',$title)," ,\t\n\r\0\x0B")?:$title;}
  $title=$feature['properties']['displayTitle']??$title;
  ...
  $type=$terms('property_type');$transaction=$terms('transaction_type');$land=($type->slug??'')==='land';
  $suppressed=in_array($id,array_map('intval',(array)get_option('aspire_atlas_suppressed_price_ids',array())),true);
  $pricing=$feature['properties']['pricing']??array('leaseRateDisplay'=>$suppressed?null:$text('lease_rate_display'),'priceDisplay'=>$suppressed?null:$text('price_display'),'salePrice'=>!$suppressed&&has_term(array('for-sale','for-sale-or-lease'),'transaction_type',$id)?get_post_meta($id,'_aspire_sale_price',true):null);
  ...
  $map=$coordinates?array('type'=>'Feature','id'=>$id,'geometry'=>array('type'=>'Point','coordinates'=>$coordinates),'properties'=>array('title'=>$title,'displayTitle'=>$title,'listingStatus'=>$text('listing_status'),'propertyType'=>array('slug'=>$type->slug??'','label'=>$type->name??''),'transactionType'=>array('slug'=>$transaction->slug??'','label'=>$transaction->name??''),'metrics'=>array(),'pricing'=>array(),'location'=>array('city'=>$city,'state'=>$state))):null;
  return compact('id','title','type','transaction','hero','groups','notes','suites','columns','gallery','brochure','brokers','address','locality','map')+array('highlights'=>self::lines($id,'property_highlights'),'has_body'=>trim((string)get_post_field('post_content',$id))!=='');
 }
}
