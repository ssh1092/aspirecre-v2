...
<section class="finder container" aria-labelledby="finder-heading">
	...
	<form class="finder-form" action="<?php echo esc_url( home_url( '/properties/' ) ); ?>" method="get">
		<div><label for="property-type">Property Type</label><select id="property-type" name="property_type"><option value="">All property types</option><?php aspirecre_taxonomy_options( 'property_type' ); ?></select></div>
		<div><label for="transaction-type">Transaction Type</label><select id="transaction-type" name="transaction_type"><option value="">All transactions</option><?php aspirecre_taxonomy_options( 'transaction_type' ); ?></select></div>
