...
<section <?php echo get_block_wrapper_attributes(array_filter(array('class'=>'aspire-atlas'.($editor?' aspire-atlas-preview':'').($corporate?' atlas-corporate':''),'id'=>$anchor))); ?> aria-labelledby="<?php echo esc_attr($uid); ?>title" <?php if(!$editor): ?>data-atlas <?php if($corporate): ?>data-corporate-hero="true"<?php endif; ?> data-worker="<?php echo esc_url(plugins_url('atlas/build/worker.js',ASPIRE_CORE_FILE)); ?>" data-endpoint="<?php echo esc_url(rest_url('aspire/v1/atlas/properties')); ?>" data-tiles="<?php echo esc_url($tiles); ?>" data-glyphs="<?php echo esc_attr($glyphs); ?>"<?php endif; ?>>
 ...
 <header class="atlas-nav">
  ...
  <nav aria-label="Primary navigation"><?php if($corporate): ?><?php foreach(array('properties'=>'Properties','services'=>'Services','about'=>'About','team'=>'Team','insights'=>'Insights') as $key=>$label): ?><a href="<?php echo esc_url($destination($key)); ?>"><?php echo esc_html($label); ?></a><?php endforeach; ?><a class="atlas-contact" href="<?php echo esc_url($destination('contact')); ?>">Talk to Aspire <span aria-hidden="true">↗</span></a><?php else: ?><a class="atlas-properties-nav" href="#<?php echo esc_attr($uid); ?>properties">Properties</a><a href="<?php echo esc_url(home_url('/insights/')); ?>">Insights</a><a href="<?php echo esc_url(home_url('/about/')); ?>">Company</a><a class="atlas-contact" href="<?php echo esc_url(home_url('/contact/')); ?>">Talk to Aspire <span aria-hidden="true">↗</span></a><?php endif; ?></nav>
 </header>
 <?php endif; ?>
 <div class="atlas-opening">
  <?php if($editor && $corporate): ?><p class="atlas-editor-label">ASPIRE ATLAS HERO</p><?php endif; ?>
