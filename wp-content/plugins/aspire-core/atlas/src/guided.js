...
import {ownerTypes,managementTypes,buildingSizes,landSizes,ownerIntents,managementNeeds,workflowValid,toggleNeed} from './workflow-data.js';
import './guided.css';
export function createGuided(root,state,onChange,onExplore,onBrief){
 ...
 function render(focus=true){
  ...
  panel.querySelector('.atlas-guided-eyebrow').textContent=`${management?'MANAGE AN ASSET':'LEASE / SELL MY PROPERTY'} · ${f.step<5?`STEP ${f.step} OF 4`:'REVIEW'}`;
  heading.textContent=f.step===1?(management?'Tell us about the property.':'Where is your property?'):f.step===2?'What type of property?':f.step===3?'Approximate size':f.step===4?(management?'What do you need help with?':"What are you considering?"):(management?'YOUR PROPERTY MANAGEMENT NEED':'YOUR PROPERTY');
  support.textContent=f.step===1?(management?'Start with the location of your property.':"Start with the location, then tell us what you're considering."):'';support.hidden=!support.textContent;
  if(f.step===1){
 ...
 next.addEventListener('click',()=>{
  ...
  if(f.step===5){onBrief();return;}
  f.step=Math.min(5,f.step+1);render();
 });
 back.addEventListener('click',()=>{flow().step=Math.max(1,flow().step-1);render();});
