	...
	<div class="hp-rail-track" id="<?php echo esc_attr( $rail_id ); ?>" data-hp-rail-track>
		<?php foreach ( $territories as $territory ) : ?>
			<article class="hp-property-type" data-hp-rail-item data-property-type="<?php echo esc_attr( $territory['slug'] ); ?>" data-property-count="<?php echo esc_attr( $territory['count'] ); ?>">
				<a class="hp-property-type-link" href="<?php echo esc_url( $territory['url'] ); ?>">
					...
					<div class="hp-property-type-copy"><h3><?php echo esc_html( $territory['name'] ); ?></h3><p><?php echo esc_html( sprintf( _n( '%s current property', '%s current properties', $territory['count'], 'aspire-core' ), number_format_i18n( $territory['count'] ) ) ); ?></p><span class="hp-property-type-arrow" aria-hidden="true">↗</span></div>
				</a>
			</article>
		<?php endforeach; ?>
	</div>
